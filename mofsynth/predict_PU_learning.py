...
def preload(preload_folder, id_prop_file):
    ...
    for cif_id in cif_list:
        pickle_path = preload_folder+'/'+cif_id+'.pickle'
        if os.path.exists(pickle_path):
            with open(pickle_path, 'rb') as f:
                data.append(pickle.load(f))
        else:
            missing_graphs.append(cif_id)

    if missing_graphs:
        print(f"\n⚠ Warning: {len(missing_graphs)} CIFs do not have crystal graphs (skipped)")
        if len(missing_graphs) <= 10:
            print(f"Missing: {', '.join(missing_graphs)}")
        else:
            print(f"First 10 missing: {', '.join(missing_graphs[:10])}")
        print(f"✓ Loaded {len(data)}/{len(cif_list)} structures with graphs\n")
